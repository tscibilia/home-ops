...
class _Server(ThreadingHTTPServer):
    # Python's default listen backlog is 5. Threaded so one slow aKeyless
    # call can't block other requests.
...
class AKeylessProxy(BaseHTTPRequestHandler):
    ...
    # Deliberately no HTTP/1.1 keep-alive: this host resolves one secret, so
    # it gains nothing, and idle connections would log a timeout every 30s.
    access_id = os.getenv("AKEYLESS_ACCESS_ID")
    access_key = os.getenv("AKEYLESS_ACCESS_KEY")
    api_url = "https://api.akeyless.io"

    def _send(self, code: int, body: bytes, ctype: str = "application/json"):
        """Send a response with an explicit Content-Length."""
