...
- **Tailscale node state is not an ExternalSecret:** `ts-ai3090-node-0` is written by `tailscaled` itself (`TS_KUBE_SECRET`). ESO must never template `_current-profile`, `_profiles`, `_machinekey` or `profile-*` into it — an hourly refresh replays a stale profile, the node re-registers under a new identity, and the pod crash-loops on `invalid key`. The auth key lives in its own secret, `ts-ai3090-node-authkey`.
