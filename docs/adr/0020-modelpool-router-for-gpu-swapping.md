...
- **Owning that port means owning the scrape endpoint.** `ServiceMonitor/llmkube-models` selects every Service labelled `inference.llmkube.dev/service` and scrapes its `http` port, which for ComfyUI is the probe rather than an inference server. A probe that serves no `/metrics` therefore keeps `TargetDown` firing for as long as the member holds the GPU, so `idle-probe.py` exposes `comfyui_up` and `comfyui_queue_depth` — the same count `/idle` gates on, which makes a deferred swap visible in Prometheus instead of only in the pool's conditions. The reverse is silent by construction: a member scaled to zero has no endpoints, so its target disappears entirely and nothing alerts on the model that is _not_ resident.
