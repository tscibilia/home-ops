...
| **namespace component** | Declared once per namespace in `kubernetes/apps/{ns}/kustomization.yaml` under `components:`, with no per-app configuration: `alerts`, `alerts/github-status`, `secrets`, `kopiur/secret`. Never appears in a `ks.yaml`.                                              |
